...
contract SecureHybridExchange is AccessControl, ReentrancyGuard {
    ...
    uint256 public swapFeeBps = 25; // 0.25% default
    ...
    // Slippage protected: user passes minAmountOut
    function swapGlobal(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut) external nonReentrant {
        require(tokenIn != tokenOut, "Identical tokens");
        require(allowedTokens[tokenIn] && allowedTokens[tokenOut], "Tokens not allowed");
        ...
        require(receiverGets >= minAmountOut, "Slippage: output too low");

        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        IERC20(tokenOut).safeTransfer(msg.sender, receiverGets);
    ...
    // Slippage protected: taker passes minAmountOut
    ...
    function cancelOrder(uint256 orderId) external nonReentrant {
        ...
        if (refundAmount > 0) {
            IERC20(ord.tokenIn).safeTransfer(msg.sender, refundAmount);
            emit OrderCancelled(orderId, refundAmount);
        }
    }

    // Partial fill with slippage protection
    function matchOrder(uint256 orderId, uint256 fillAmount, uint256 minAmountOut) external nonReentrant {
        require(orderId < orders.length, "Invalid");
        Order storage ord = orders[orderId];
        require(ord.active, "Inactive");
        require(tokenInOutAllowed(ord.tokenIn, ord.tokenOut), "Tokens not allowed");
        ...
        require(takerGets >= minAmountOut, "Slippage: output too low");
        require(takerGets > 0, "Too small");

        // Take tokenOut from taker, reward maker
        IERC20(ord.tokenOut).safeTransferFrom(msg.sender, address(this), tokenOutAmount);
        IERC20(ord.tokenOut).safeTransfer(ord.maker, takerGets);
        if (fee > 0) IERC20(ord.tokenOut).safeTransfer(feeRecipient, fee);

        // Send tokenIn to taker
        IERC20(ord.tokenIn).safeTransfer(msg.sender, fillAmount);
    ...
    // Helper
    function tokenInOutAllowed(address tokenIn, address tokenOut) internal view returns(bool) {
        return tokenIn != tokenOut && allowedTokens[tokenIn] && allowedTokens[tokenOut];
    }
}
